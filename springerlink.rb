...
class Springerlink
  ...
  def get_article_hash(document)
    ...
    article_hash = {}
    document.css('a.title').map { |article| article_hash[article['href']] = article.text.gsub(/[\x00\/\\:\*\?\"<>\|„“]/, '') }
    article_hash.keys.map.with_index { |article,i| article_hash[["http://link.springer.com/content/pdf/", article.delete("/article").sub(/s/, "%2Fs")]] = [ published_dates[i].attributes["title"].value.split.reverse, article_hash.delete(article) ] }
    article_hash
  end
  ...
  def download_articles(article_hash, document)
    article_hash.each do |link|
      ...
      `wget #{prefix+article}`
      `mv #{article} '#{journal}/#{year}/#{month}/#{title}.pdf'`
    end
  end
end
